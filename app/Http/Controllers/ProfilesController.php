...
class ProfilesController extends Controller
{
    ...
    public function profile_validator(array $data)
    {
        return Validator::make($data, [
            ...
            'avatar'            => '',
            'postcode'          => 'required|min:2|max:50',
            'avatar_status'     => '',
            'street'            => 'required|min:2|max:50',
            'city'              => 'required|min:2|max:50',
            'country'           => 'required|min:2|max:50',
            'birthday'          => 'required|min:2|max:50',
            'birthplace'        => 'required|min:2|max:50',
            'fixed_number'      => 'required|min:2|max:50',
            'mobile_number'     => '',
            'homepage'          => '',
            'vita'              => 'min:2|max:10000',
            'exhibition'        => 'min:2|max:10000',
            'newsletter'        => '',
            'ideenstorming'     => '',
            'profile_public'     => ''
        ]);
    ...
    public function edit($username)
    {
        ...
        if ($user->profile->country) {
            $selected_value = $user->profile->country;
        } else {
            $selected_value = 'DE';
        }

        return view('profiles.edit')->with($data)->with('countries', $countries)->with('selected_value', $selected_value);
    }
    ...
    public function update($username, Request $request)
    {
        ...
        if (isset($input['ideenstorming'])) {
            $input['ideenstorming'] = 1;
        } else {
            $input['ideenstorming'] = 0;
        }

        if (isset($input['profile_public'])) {
            $input['profile_public'] = 1;
        } else {
            $input['profile_public'] = 0;
        }
